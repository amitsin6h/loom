...
<script>
    ...
    $('#to_date').datepicker({
	  ...
	  function get_report()
	  {
		  ...
		  var myKeyVals="to_do=get_report&emp_code="+emp_code+"&submit=Get Report&from_date="+from_date+"&to_date="+to_date+"&shift="+shift+"&loom_no="+loom_no+"&quick_tab="+quick_tab;		  
		  var saveData = $.ajax({
			type: 'POST',
			url: "login.php",
			data: myKeyVals,
    ...
    $('#example1').DataTable( {
        ...
        "ajax": 'login.php?to_do=get_report',
		"dom": 'Bfrtip',
		"buttons": [
            {
                extend: 'excelHtml5',
...
function clear_sam(){
	...
	function del(elem){
		var Id = $(elem).data('id');
		var myKeyVals="to_do=Delete&Id="+Id+"&submit=Del";	
		var table = $('#example1').DataTable();
		var saveData = $.ajax({
			type: 'POST',
			url: "login.php",
